...
function viewUserDetails(){
	listItems();
	 ...
	 jQuery.ajax
		({
			type: 'POST',
			url: baseUrl+'/adURL/getAdURLData',
			datatype:'jsonp',
			jsonp:"callback",
			data:{"id":id},
			success: function(response){
			console.log(response);
			var readyData=(response);							
			 //$('#totalData').text("");		 
			for(var i=0;i<readyData.length;i++)
			{	
				var addata="<tr>"+
					"<td>"+(i+1)+"</td>"+
					 "<td>"+readyData[i].FN+"</td>"+
					 "<td>"+readyData[i].LN+"</td>"+
					 "<td>"+readyData[i].AGE+"</td>"+
					 "<td>"+0+readyData[i].ZIP+"</td>"+
					 "<td>"+readyData[i].GENDER+"</td>"+
					 "<td>"+readyData[i].HH_MARITAL_STAT+"</td>"+
					 "<td>"+readyData[i].HH_INCOME+"</td>"+									 
					 "<td >"+readyData[i].HOME_MKT_VALUE+"</td>"+
					 "<td >"+readyData[i].CREDIT_RATING+"</td>"+
					 "</tr>"									
					$('#totalData').append(addata); 				
			}   												
			},
		...
		jQuery.ajax
			({
				...
				type: 'POST',
				url: baseUrl+'/adURL/getAdURLName',
				datatype:'jsonp',
				jsonp:"callback",
				data:{"id":id},
				success: function(response){
					 $('#adtitle').text(""); 
					 console.log(response[0].adURL);
					$("#adtitle").append(response[0].adURL);
				},				
				error: function(error){
					console.log("sorry");
				}
...
function viewMetrics()
	 {
			   ...
			   {	
					window.location = "viewUserDetails.html?id="+id;
   ...
   var cb = function(start, end)
	{
		...
		var d1=new Date(y);
		console.log(d1);
		/* d1.setUTCHours(23,59,59,0); */
		d1=d1.toISOString();
		end._d=d1;
		console.log(d1);
		var date13=new Date(d1);
		end._d=date13;
