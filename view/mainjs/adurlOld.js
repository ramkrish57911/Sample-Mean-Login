			   ...
			   if(ownerrenter == null || ownerrenter == "Select Position")
				...
				if(result == 0)
				{
				  ...
				  var myForm = new FormData();
				   ...
				   myForm.append("houseValueMoney",$('#houseValueMoney').val());
				   myForm.append("incomeGroupValue",$('#incomeGroupValue').val());
								...
								var adurl="<tr class='even pointer'>"+
									 ...
									 "<td>"+response[i].adURL+"</td>"+
									  ...
									  "<td >"+response[i].zip+"</td>"+
									   "<td class='last'><a id='"+response[i]._id+"' onClick='editUrl(this.id)'  style='cursor:pointer' title='Edit'><i class='fa fa-edit'></i></a>&nbsp"+
										"<a id='"+response[i]._id+"' onClick='deleteAdurl(this.id)' style='cursor:pointer' title='Delete'><i class='fa fa-trash'></i></a>"+
			   ...
			   function editUrl(adurlId)
			    {
					...
					({
							...
							success: function(response)
							{
								...
								$('#houseValueMoney').val(response[0].houseValueMoney);
								$('#incomeGroupValue').val(response[0].incomeGroupValue);
